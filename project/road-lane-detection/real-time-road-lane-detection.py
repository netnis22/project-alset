...
def showImages(frame, mask, edges, roi, res, line_image,debug):
...
def region_of_interest(edges):
    ...
    polygons1 = np.array([[(0, height), ((320), height), (0, (125))]])
    polygons2 = np.array([[(0, (125)), ((320), height), ((320), (125))]])
...
d = {'V10T': 0, 'V0T': 0, 'V0T': 0, 'V0T': 0, 'V7T': 0, 'V7T': 0, 'error': 0}
...
def errorTurn(left_fit, right_fit, image):
    ...
    if(left and right):
        turnError=((midelOfRode(left_line, right_line)[0])-(320/2))/(320)
...
def checkAndPrint(left_fit, right_fit, image):
    ...
    if (left and right and ((310/2) < midelOfRode(left_line, right_line)[0] < (330/2))) and (
        ...
        msg = 'V0T' #RIGHT
    elif midelOfRode(left_line, right_line)[0] > (330/2):
        msg = 'V7T' #right + drive
    elif (310/2) > midelOfRode(left_line, right_line)[0]:
        msg = 'V7T' #left + drive
    else:
        msg = 'error'
...
def main():
    ...
    while flag & cap.isOpened():
        ...
        #Denoising = roi
        #cv.fastNlMeansDenoising(roi,Denoising,5,30)
        edges = cv.Canny(roi, 0, 400)  # Prepares an image only at the edges of the mask
        debug = region_of_interest(edges)
        ...
        if counter > 2:
            ...
            for i in d.keys():
                d[i] = 0
            print((f'{msg}{turnError*-1}')+'_')
            #communication
            buff = ((f'{msg}{turnError*-1}')+'_').encode()
            #buff=('v0').encode()
            ser.write(buff)

        line_image = display_lines(frame, averaged_lines)
        res = cv.addWeighted(frame, 0.8, line_image, 1, 1)
        showImages(frame, mask, edges, roi, res, line_image,debug)  # Activates the function showImages
